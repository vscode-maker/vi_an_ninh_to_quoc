const fs = require('fs');
const path = require('path');

const envPath = path.join(__dirname, '.env');

// Credentials provided by user
const clientEmail = 'app-uploader@vianninhtoquoc-482816.iam.gserviceaccount.com';
const folderId = '1jqdDUujfJHgZI3ekAPZ972mkFpmGNzDo'; // Extracted from screenshot URL
const privateKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

// Convert the multiline private key to a single line with \n literal
const privateKeyOneLine = privateKey.replace(/\r\n/g, '\n').replace(/\n/g, '\\n');

try {
    let content = '';
    if (fs.existsSync(envPath)) {
        content = fs.readFileSync(envPath, 'utf8');
    }

    const lines = content.split('\n');
    const newLines = [];
    const foundKeys = {
        'GOOGLE_CLIENT_EMAIL': false,
        'GOOGLE_PRIVATE_KEY': false,
        'GOOGLE_DRIVE_FOLDER_ID': false
    };

    lines.forEach(line => {
        const parts = line.split('=');
        if (parts.length >= 2) {
            const key = parts[0].trim();
            if (foundKeys.hasOwnProperty(key)) {
                // Skip existing logic, we will append new values
                foundKeys[key] = true;
                // Don't push the old line
                return;
            }
        }
        newLines.push(line);
    });

    // Append new values
    newLines.push(`GOOGLE_CLIENT_EMAIL="${clientEmail}"`);
    newLines.push(`GOOGLE_PRIVATE_KEY="${privateKeyOneLine}"`);
    newLines.push(`GOOGLE_DRIVE_FOLDER_ID="${folderId}"`);

    const newContent = newLines.filter(line => line.trim() !== '').join('\n'); // Remove empty lines to be clean

    fs.writeFileSync(envPath, newContent);
    console.log('Successfully updated .env file');

} catch (e) {
    console.error('Error updating .env:', e);
}
